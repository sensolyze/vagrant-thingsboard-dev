...
#--- Get the VM config from the ansible file, and set up the "hosts" variable
if is_windows
  json = %x(ruby ./bin/inventory.rb --list)
else
  json = %x(bin/inventory.rb --list)
end
...
hosts = Hash.new
inventory["vagrant-kitchen"].each do |host|
